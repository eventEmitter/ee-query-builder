!function(){
    ...
    module.exports = new Class({
        ...
        */
         ...
         */
        , _renderFilter: function(parameters, filter, property, entity, dontEscape) {
            var   items = []
                , result
                , id;
            ...
            switch (type(filter)) {
                case 'array':
                    filter.forEach(function(filterItem) {
                        result = this._renderFilter(parameters, filterItem, (property === '_' ? (entity || property) : (property || entity)), entity, dontEscape);
                ...
                case 'object':
                    if (type.function(filter.isQuery) && filter.isQuery()) {
                        ...
                        // returns it
                        return (entity ? (this._escapeId(entity) + '.') : '') + (dontEscape ? (property || '') : this._escapeId(property || ''))+' = ('+this._render('query', this._prepareQuery(filter), parameters).SQLString+')';
                    }
                    else {
                        Object.keys(filter).forEach(function(name) {
                            result = this._renderFilter(parameters, filter[name], name, property, dontEscape);
                            if (result !== undefined) items.push(result);
                        }.bind(this));
                ...
                case 'date':
                case 'boolean':
                    return (entity ? (this._escapeId(entity) + '.') : '') + (dontEscape ? (property || '') : this._escapeId(property || '')) + ' = '+parameters.set(property, filter, true);


                case 'null':
                case 'undefined':
                    return (entity ? (this._escapeId(entity) + '.') : '') + (dontEscape ? (property || '') : this._escapeId(property || '')) + ' is null';


                case 'function':
                    return this._renderCommand(property, filter(), parameters, (entity ? (this._escapeId(entity ) + '.') : ''), dontEscape);
         ...
         * @param <String>  entity to apply the current filter on
         */
        , _renderCommand: function(property, command, parameters, entity, dontEscape) {
            var result;
            ...
            if (command.operator) {
                ...
                if (command.query) {
                    return entity+(dontEscape ? property : this._escapeId(property)) + ' ' + this._operators[command.operator] + ' (' + this._render('query', command.query, parameters).SQLString +')';
                }
                else if (command && command.value && type.function(command.value.isQuery) && command.value.isQuery()) {
                    return entity+(dontEscape ? property : this._escapeId(property))+' '+this._operators[command.operator]+ ' (' + this._renderSubQuery(command.value, parameters) +')';
                }
                else {
                    return entity+(dontEscape ? property : this._escapeId(property)) + ' ' + this._operators[command.operator] + ' '+parameters.set(property, command.value, true);
                }
            }
            ...
            // function
                ...
                result = this._functions[command.fn](command, parameters, property, entity);
                ...
                // normal filter
                    ...
                    if (command.rightSide) return this._renderFilter(parameters, command.value, result, '', true);
                    else return entity+(dontEscape ? property : this._escapeId(property)) + ' ' + result;
                }
