    ...
    module.exports = new Class({
        ...
        , _renderGroup: function(group) {
            return group.map(function(grouping) {
                ...
                else return this.escapeId(grouping.table || '') + '.' + this.escapeId(grouping.column || '');
            }.bind(this)).join(', ');
        }
        ...
        */
         ...
         */
        , _renderFilter: function(filter, property, entity, dontEscape) {
            var   items = []
                , result
                , id;
            ...
            switch (type(filter)) {
                case 'array':
                    filter.forEach(function(filterItem) {
                        result = this._renderFilter(filterItem, (property === '_' ? (entity || property) : (property || entity)), entity, dontEscape);
                ...
                case 'object':
                    ...
                    else if (filter.type && filter.type === 'fulltext') {
                        return this.renderFullTextSearch(filter, property, entity, dontEscape);
                    }
                    else {
                        Object.keys(filter).forEach(function(name) {
                            result = this._renderFilter(filter[name], name, property, dontEscape);
                            if (result !== undefined) items.push(result);
                        }.bind(this));
        ...
        , renderFullTextSearch: function(filter, property, entity, dontEscape) {
            return `${(entity ? (this.escapeId(entity) + '.') : '')}${(dontEscape ? (property || '') : '')} @@ to_tsquery(${this.renderFullTextComponent(filter.value)})`;
        }
        ...
        , renderFullTextComponent(element) {
            switch (element.type) {
                ...
                case  'or':
                    return element.children.map(child => this.renderFullTextComponent(child)).join(' | ');
                case  'value':
                    let value = this.escape(element.value);

                    if (element.wildcardBefore) value = `*:${value}`;
                    if (element.wildcardAfter) value = `${value}:*`;
                    if (element.not) value = `! ${value}`;
                    return value;
            }
        }
        ...
        , _renderCommand: function(property, command, entity, dontEscape) {
            ...
            // function
                ...
                // normal filter
                    ...
                    if (command.rightSide) return this._renderFilter(command.value, result, '', true);
