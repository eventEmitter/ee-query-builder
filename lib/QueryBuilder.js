    ...
    module.exports = new Class({
        ...
        */
            ...
            switch (type(filter)) {
                case 'array':
                    ...
                    filter.forEach((filterItem) => {
                        const result = this._renderFilter(query, filterItem, (property === '_' ? (entity || property) : (property || entity)), entity, dontEscape);
                ...
                case 'object':
                    if (type.function(filter.isQuery) && filter.isQuery()) {
                        ...
                        // normal filter hierarchy -> nested objects describing
                        // entities and properties
                        const items = [];


                        Object.keys(filter).forEach((name) => {
                            const result = this._renderFilter(query, filter[name], name, property, dontEscape);
                            if (result !== undefined) items.push(result);
                        });
                ...
                default:
                    // looks like we're getting way too often to this point
                    // hence we need to print a shitload of debug info
                    log.info('query -> ', query);
                    log.info('entity -> ', entity);
                    log.info('property -> ', property);
                    log.info('filter -> ', filter);
                    log.info('typeof filter, type(filter) -> ', typeof filter, type(filter));
                    console.log('filter (console.log) ->', filter);
                    throw new Error('Cannot process the type «'+type(filter)+'» in the query builder!');
            }
        }
        ...
        , _renderCommand: function(property, command, entity, dontEscape) {
            ...
            if (command.operator) {
                ...
                if (command.query) {
                    ...
                    if (command.value === null || command.value === undefined) return entity+(dontEscape ? property : this.escapeId(property)) + ' is null';
        ...
        , renderSelectFunction: function(tablename, command) {
            ...
            // if the command is a function we need to render it
            if (type.function(command.value)) {
                const renderedValue = this._renderSelect([command.value], tablename);
                return this.aggregateFunctions[command.fn]+'('+renderedValue+')'+(command.alias ? ' AS '+this.escapeId(command.alias) : '');
            } else {
        ...
        , in: function(command) {
            ...
            if (type.function(values.isQuery)) {
                ...
                if (values.length) {
                    return ' IN (' + Array.from(new Set(values)).map(value => this.renderInValueValue(value)).join(', ') +')';
                } else return false;
            }
        }
        ...
        , notIn: function(command) {
            ...
            if (type.function(values.isQuery)) {
                ...
                if (values.length) {
                    return ' NOT IN (' + Array.from(new Set(values)).map(value => this.renderInValueValue(value)).join(', ') +')';
                } else return undefined;
            }
        }
